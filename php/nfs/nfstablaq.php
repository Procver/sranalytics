...
for ($i = 0; $i < count($arr["Result"]); $i++) {
    try {
        if ($arr["Result"][$i]["TotalTime"] != 0) {
            ...
            // Armando array
            $arrQualy[$i] = ["Posicion" => strval($posicion), "Piloto" => $arr['Result'][$i]['DriverName'], "Diferencia" => $Diferencia, "MVuelta" => $mejorVuelta, "Sector1" => $sector1, "Sector2" => $sector2,"Sector3" => $sector3];
        }
    } catch (\Exception $e) {
    }
}
